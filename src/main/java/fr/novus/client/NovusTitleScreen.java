...
public final class NovusTitleScreen extends Screen {
    ...
    private void loadMods() {
        ...
        FabricLoader.getInstance().getAllMods().forEach(mod -> {
            ...
            loaded.add(name == null || name.isBlank() ? mod.getMetadata().getId() : name);
        });
    ...
    private void layout() {
        ...
        int actionY = h - 30;
        playX = contentLeft; playY = actionY; playW = actionW; playH = 15;
        secondaryX = playX + actionW + GAP; secondaryY = actionY; secondaryW = actionW; secondaryH = 15;
        quitX = 12; quitY = h - 23; quitW = SIDEBAR - 24; quitH = 15;
    }
    ...
    private void drawSidebar(DrawContext ctx, int mouseX, int mouseY, int h) {
        ...
        for (int i = 0; i < NAV.length; i++) {
            ...
            if (active || hover) ctx.fill(10, top - 4, SIDEBAR - 10, top + 24, active ? 0xFF222A35 : 0xFF1A2028);
            if (active) ctx.fill(10, top - 4, 13, top + 24, NovusUi.accent());
            ctx.drawText(tr, Text.literal("0" + (i + 1)), 18, top + 5, active ? NovusUi.accent() : NovusUi.muted(), false);
            ctx.drawText(tr, Text.literal(NAV[i]), 42, top + 5, active ? NovusUi.text() : NovusUi.muted(), false);
        }
        NovusUi.button(ctx, tr, quitX, quitY, quitW, quitH, "QUITTER", mouseX, mouseY, false);
    }

    private void drawContent(DrawContext ctx, int mouseX, int mouseY, int w, int h) {
        TextRenderer tr = this.textRenderer;
        int x = SIDEBAR + MARGIN, right = w - MARGIN, top = 18;
        ctx.drawTextWithShadow(tr, Text.literal(title()), x, top, NovusUi.text());
        ctx.drawText(tr, Text.literal(subtitle()), x, top + 15, NovusUi.muted(), false);
    ...
    private void drawModpack(DrawContext ctx, int x, int right, int top) {
        NovusUi.panel(ctx, x, top, right, top + 112, true);
        ...
        int y = top + 55, max = Math.min(mods.size(), 4);
        for (int i = 0; i < max; i++) ctx.drawText(textRenderer, Text.literal("• " + mods.get(i)), x + 18, y + i * 12, NovusUi.muted(), false);
        if (mods.size() > max) ctx.drawText(textRenderer, Text.literal("+ " + (mods.size() - max) + " autre(s)"), x + 18, y + max * 12, NovusUi.accent(), false);
    }
    ...
    private void drawServer(DrawContext ctx, int x, int right, int top) {
        ...
        ctx.drawText(textRenderer, Text.literal("Le bouton ouvre directement la liste des serveurs."), x + 18, top + 87, NovusUi.muted(), false);
    }

    private void drawSettings(DrawContext ctx, int x, int right, int top) {
        NovusUi.panel(ctx, x, top, right, top + 112, false);
    ...
    private void drawActions(DrawContext ctx, int mouseX, int mouseY) {
        TextRenderer tr = this.textRenderer;
        if (page == 0) {
            NovusUi.button(ctx, tr, playX, playY, playW, playH, "JOUER", mouseX, mouseY, true);
            NovusUi.button(ctx, tr, secondaryX, secondaryY, secondaryW, secondaryH, "SERVEURS", mouseX, mouseY, false);
        } else if (page == 1) {
            NovusUi.button(ctx, tr, playX, playY, playW, playH, "JOUER", mouseX, mouseY, true);
            NovusUi.button(ctx, tr, secondaryX, secondaryY, secondaryW, secondaryH, "RECHARGER", mouseX, mouseY, false);
        } else if (page == 2) {
            NovusUi.button(ctx, tr, playX, playY, playW, playH, "SERVEURS", mouseX, mouseY, true);
            NovusUi.button(ctx, tr, secondaryX, secondaryY, secondaryW, secondaryH, "ACCUEIL", mouseX, mouseY, false);
        } else {
            NovusUi.button(ctx, tr, playX, playY, playW, playH, "OPTIONS", mouseX, mouseY, true);
            NovusUi.button(ctx, tr, secondaryX, secondaryY, secondaryW, secondaryH, "ACCUEIL", mouseX, mouseY, false);
        }
    }

    private void drawFooter(DrawContext ctx, int w, int h) {
        String left = "NOVUS CLIENT  •  v0.1.0", right = "GUI LOCAL 3x";
    ...
    private String title() { return NAV[page]; }
    private String subtitle() { return switch (page) {
    ...
    @Override public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        double x = mouseX / UI_SCALE, y = mouseY / UI_SCALE;
        for (int i = 0; i < NAV.length; i++) {
            int top = 63 + i * 35;
            if (NovusUi.inside(10, top - 4, SIDEBAR - 10, top + 24, x, y)) { page = i; return true; }
        }
        if (NovusUi.inside(quitX, quitY, quitX + quitW, quitY + quitH, x, y)) { if (client != null) client.scheduleStop(); return true; }
        if (NovusUi.inside(playX, playY, playX + playW, playY + playH, x, y)) {
            switch (page) { case 0, 1 -> openSingleplayer(); case 2 -> openMultiplayer(); case 3 -> openOptions(); default -> { } }
            return true;
        }
        if (NovusUi.inside(secondaryX, secondaryY, secondaryX + secondaryW, secondaryY + secondaryH, x, y)) {
            switch (page) { case 0 -> openMultiplayer(); case 1 -> loadMods(); case 2, 3 -> page = 0; default -> { } }
            return true;
        }
